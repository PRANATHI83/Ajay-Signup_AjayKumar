...
// Login route (updated to return JWT)
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Generate JWT
    const token = jwt.sign({ email: user.email, userId: user.id }, JWT_SECRET, { expiresIn: '8h' });
...
// Fetch user details (protected with JWT)
app.get('/api/user', authenticateToken, async (req, res) => {
  try {
    const email = req.user.email; // From JWT
    const result = await pool.query('SELECT id, username, email, profile_image, created_at FROM users WHERE email = $1', [email]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'User not found' });
    }
